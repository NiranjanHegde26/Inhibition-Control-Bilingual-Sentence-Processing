(function () {
  var app = angular.module("SquaredFlankerExperimentApp", ["Lingoturk"]);
  ...
  app.controller("RenderController", [
    ...
    function ($http, $timeout, $scope, $interval) {
      ...
      self.practiceTrialQuestions = Array(10).fill(stimuli_flanker).flat();
      ...
      this.submitResults = function (successCallback, errorCallback) {
        var results = {
          experimentType: "SquaredFlankerExperiment",
      ...
      this.pausePracticeCountdown = function () {
        if (practiceCountdownPromise) {
          $interval.cancel(practiceCountdownPromise);
          practiceCountdownPromise = null;
          $scope.isPaused = true;
      ...
      this.resumePracticeCountdown = function () {
        if ($scope.isPaused) {
      ...
      this.savePracticeAnswer = function (ans) {
        ...
        if ($scope.correct) {
          ...
          var delay = 500; // shorter delay for correct
        } else {
          $scope.practiceScore -= 1;
